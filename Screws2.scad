...
module bolthead(radius,thickness,sides){
   intersection(){
   cylinder(thickness,radius,5,$fn=sides);
   cylinder(thickness,radius*.95,radius*.95,$fn=300);
   }
}
difference(){
   bolthead(5,3,6);
...
//translate([0,0,2]) cylinder(30,2,2,$fn=30);
translate([0,0,3]) metric_thread(8,3,29.8,n_starts=2);
//translate([0,-10,0]) metric_thread(10,4,10,n_starts=10);
//translate([0,10,0]) english_thread(1,3,1,n_starts=10);

translate([0,10,3])  difference(){
        cylinder(27,5,5);
        metric_thread(8,3,29.8,n_starts=2);
}
